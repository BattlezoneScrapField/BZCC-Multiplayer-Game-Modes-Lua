...
function GetInitialPlayerPilotODF(Race)
    if (Mission.m_RespawnWithSniper) then
        return Race .. "spilo"   -- With sniper.
    else
        return Race .. "suser_m" -- Note-- this is the sniper-less variant for a respawn
    end
end
...
function GetInitialRecyclerODF(Race)
    ...
    if (pContents ~= nil and pContents ~= "") then
        return Race .. string.sub(pContents, 2)
    else
        return Race .. "vrecy_m"
    end
end
...
function Start()
    ...
    Mission.m_RecyInvulnerabilityTime = Mission.m_RecyInvulnerabilityTime *
        (60 * m_GameTPS) -- convert from minutes to ticks (10 ticks per second)
...
function ExecuteRecyInvulnerability()
    ...
    for i = 1, MAX_TEAMS - 1 do
        if (Mission.m_TeamIsSetUp[i]) then
            ...
            if (recyHandle ~= nil and _DLLUtils.IsRecycler(recyHandle)) then
    ...
    if (Mission.m_RecyInvulnerabilityTime == 1 or Mission.m_RecyInvulnerabilityTime % (60 * m_GameTPS) == 0 or Mission.m_RecyInvulnerabilityTime < (60 * m_GameTPS) and Mission.m_RecyInvulnerabilityTime % (15 * m_GameTPS) == 0) then
        if (Mission.m_RecyInvulnerabilityTime == 1) then
            TempMsgString = TranslateString("strat", "recyclersVulnerable")
        elseif (Mission.m_RecyInvulnerabilityTime > (60 * m_GameTPS)) then
            TempMsgString = TranslateString("strat", "invulnerableExpiresMin"):format(Mission.m_RecyInvulnerabilityTime /
                (60 * m_GameTPS))
        else
            TempMsgString = TranslateString("strat", "invulnerableExpiresSec"):format(Mission.m_RecyInvulnerabilityTime /
                m_GameTPS)
        end
...
function ExecuteCheckIfGameOver()
    ...
    for i = 1, MAX_TEAMS - 1 do
        if (Mission.m_TeamIsSetUp[i]) then
            ...
            if (not Mission.m_NotedRecyclerLocation[i] or not bit32.band(Mission.m_ElapsedGameTime, 0xFF)) then
                -- Grab out allie's Recy for respawn placement if ours is dead.
                if (RecyH == nil) then          -- Uh oh, your recy+factory are dead. Look for ones on allied teams.
            ...
            if (RecyH ~= nil) then -- The above loop found something, update respawn positions.-GBD
    ...
    if (NumFunctioningTeams == 0 and Mission.m_ElapsedGameTime > (5 * m_GameTPS)) then
        ...
        if (Mission.m_HadMultipleFunctioningTeams and NumFunctioningTeams == 1) then
            ...
            if (IsTeamplayOn()) then
                ...
                for i = 1, MAX_TEAMS - 1 do -- Start at 0. Also give a more proper message if allies are on. -GBD
                    if (AlliesFunctioning[Mission.m_AllyTeams[i]]) then
                        if (Mission.m_HasAllies[i]) then
                            local TeamName = TranslateString("cfg", "Team") ..
                                " " ..
                                Mission.m_AllyTeams
                                [i]                                      -- Tell who won. -GBD
                            ...
                            for j = 1, j < MAX_TEAMS - 1 do              -- Give score to allies. -GBD
...
function DeadObject(DeadObjectHandle, KillersHandle, isDeadPerson, isDeadAI)
    ...
    if (deadObjectTeam ~= 0) then
        ...
        if (isSpawnKill) then
            TempMsgString = TranslateString("mission", "Spawn kill by %s on %s\n"):format(GetPlayerName(KillersHandle),
                GetPlayerName(DeadObjectHandle))
            AddToMessagesBox(TempMsgString)
