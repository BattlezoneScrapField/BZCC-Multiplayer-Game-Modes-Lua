...
-- Constants, won't change at runtime.
local VEHICLE_SPACING_DISTANCE = 20.0

-- How many seconds sniped AI craft will stick around before going boom.
...
function GetCanEject(h)
    ...
    if (Mission.m_MissionType == DMSubtype_Normal or Mission.m_MissionType == DMSubtype_KOH or Mission.m_MissionType == DMSubtype_Loot) then
        return true
    else
        return false
    end
...
function FindGoodAITarget(index)
    ...
    if (not IsAliveAndPilot(Mission.m_AICraftHandles[index])) then
        Mission.m_AICraftHandles[index] = nil
        return
    end

    -- Rabbit mode? Hammer the "it" player, nothing else to do
    if (Mission.m_RabbitMode and Mission.m_AICraftHandles[index] ~= Mission.m_RabbitTargetHandle) then
        if (IsAlive(Mission.m_RabbitTargetHandle)) then
    ...
    if (nearestEnemy ~= nil and not Mission.m_AILastWentForPowerup[index]) then
    ...
    if (nearestEnemy == nil) then
        ...
        if (not Mission.m_HumansVsBots) then
            -- Scan botlist..
            for i = 0, Mission.m_NumAIUnits - 1 do
                -- Can't attack self.
                if (i == index) then
                    break
                end
...
function Start()
    ...
    if (Mission.m_MaxAnimals > 0) then
        Mission.m_AnimalConfig = _DLLUtils.GetCheckedNetworkSvar(12, NETLIST_Animals)
    ...
    if (ImServer() or not IsNetworkOn()) then
        ...
        if (Mission.m_RabbitMode and Mission.m_RabbitTargetHandle == nil) then
...
function ObjectifyRacePoint()
    ...
    for i = 0, Mission.m_TotalCheckpoints do
    ...
    if (Idx >= 0) then
        if (not Mission.m_TotalRaceLaps or (Mission.m_LapNumber[Idx] < Mission.m_TotalRaceLaps)) then
...
function ExecuteRabbit()
        ...
        if (not IsAround(RabbitH)) then
            ...
            if (IsAround(RabbitH)) then
                ...
                for i = 1, MAX_TEAMS - 1 do
                    ...
                    if (T2 ~= 0 and PlayerH ~= nil and T2 ~= Mission.m_ForbidRabbitTeam) then
                        SetNewRabbit(PlayerH)
                        foundNewRabbit = true
                ...
                if (not foundNewRabbit) then
                    for i = 1, MAX_AI_UNITS - 1 do
                        local index2 = (Mission.m_ElapsedGameTime + i) % MAX_AI_UNITS

                        if (Mission.m_AICraftHandles[index2] ~= nil) then
    ...
    if (LastShooter ~= nil and LastShooter ~= Mission.m_RabbitTargetHandle and LastShooter ~= Mission.m_RabbitShooterHandle) then
    ...
    if (Mission.m_ElapsedGameTime % (5 * m_GameTPS) == 0 and (CountPlayers() > 1 or Mission.m_NumAIUnits > 0)) then
        AddScore(Mission.m_RabbitTargetHandle, 1) -- Staying alive....
    end

    if (not (Mission.m_ElapsedGameTime % m_GameTPS)) then
        ObjectifyRabbit()
    end
end
...
function ExecuteRace()
    if (not RaceSetObjective or not (Mission.m_ElapsedGameTime % m_GameTPS)) then
    ...
    for i = 1, MAX_TEAMS - 1 do
        ...
        if (PlayerH ~= nil) then
            ...
            if (NextCheckpoint ~= nil) then
                ...
                if (GetDistance(PlayerH, NextCheckpoint) < Mission.m_RaceCheckpointRadius and (not Mission.m_TotalRaceLaps or Mission.m_LapNumber[i] < Mission.m_TotalRaceLaps)) then
                    ...
                    if (Mission.m_NextRaceCheckpoint[i] == 1 and i == GetLocalPlayerTeamNumber()) then
                        if (Mission.m_TotalRaceLaps ~= 0) then
                            StaticTempMsgStr = TranslateString("mission",
                                ("Lap %d/%d Completed"):format(Mission.m_LapNumber[i], Mission.m_TotalRaceLaps))
                        else
                            StaticTempMsgStr = TranslateString("mission",
                                ("Lap %d Completed"):format(Mission.m_LapNumber[i]))
                        end
            ...
            if (Advanced[i]) then
                ...
                if (LeadingPlayer) then
                    if (i ~= Mission.m_LastTeamInLead) then
                        StaticTempMsgStr = TranslateString("mission",
                            ("%s takes the lead"):format(GetPlayerName(PlayerH)))
                        AddToMessagesBox(StaticTempMsgStr)
                ...
                if (LeadingPlayer and Mission.m_NextRaceCheckpoint[i] == 1) then
                    ...
                    if (Mission.m_TotalRaceLaps ~= 0) then
                        if (not Mission.m_RaceWinerCount) then
                            StaticTempMsgStr = TranslateString("mission",
                                ("Lap %d/%d completed by %s"):format(Mission.m_LapNumber[i], Mission.m_TotalRaceLaps,
                                    GetPlayerName(PlayerH)))
                        end
                    else
                        if (not Mission.m_RaceWinerCount) then
                            StaticTempMsgStr = TranslateString("mission",
                                ("Lap %d completed by %s"):format(Mission.m_LapNumber[i], GetPlayerName(PlayerH)))
                        end
                    end
                    ...
                    if (Mission.m_LapNumber[i] == Mission.m_TotalRaceLaps) then
                        ...
                        if (Mission.m_RaceWinerCount == 1) then
                            StaticTempMsgStr = TranslateString("mission",
                                ("%s finished in 1st place"):format(GetPlayerName(PlayerH)))
                            AddScore(PlayerH, 100)
                        elseif (Mission.m_RaceWinerCount == 2) then
                            StaticTempMsgStr = TranslateString("mission",
                                ("%s finished in 2nd place"):format(GetPlayerName(PlayerH)))
                            AddScore(PlayerH, 75)
                        elseif (Mission.m_RaceWinerCount == 3) then
                            StaticTempMsgStr = TranslateString("mission",
                                ("%s finished in 3rd place"):format(GetPlayerName(PlayerH)))
                            AddScore(PlayerH, 50)
                        else
                            StaticTempMsgStr = TranslateString("mission",
                                ("%s finished in %dth place"):format(GetPlayerName(PlayerH), Mission.m_RaceWinerCount))
                            AddScore(PlayerH, 25)
                        end
...
function ExecuteWeenie()
    ...
    for i = 0, Mission.m_NumAnimals - 1 do
    ...
    for i = 0, Mission.m_NumVehiclesTracked - 1 do
...
function UpdateAIUnits()
    ...
    if (Mission.m_ElapsedGameTime % InitialSpawnInFrequency == 0) then
        if (Mission.m_NumAIUnits < Mission.m_MaxAIUnits) then
            ...
            for i = 0, Mission.m_NumAIUnits - 1 do
                ...
                if (not IsNotDeadAndPilot(Mission.m_AICraftHandles[i])) then
                    SetLifespan(Mission.m_AICraftHandles[i], SNIPED_AI_LIFESPAN)
                    Mission.m_AICraftHandles[i] = nil
                end

                if (Mission.m_AICraftHandles[i] == nil) then
                    BuildBotCraft(i)
                    break
                end
            end
    ...
    for i = 0, Mission.m_NumAIUnits - 1 do
        if (bit32.band((Mission.m_ElapsedGameTime + i), 0x1F) == 0) then
            if (Mission.m_AILastWentForPowerup[i]) then
                ...
                if (not IsAlive(Target) or Mission.m_PowerupGotoTime[i] > 150 or GetDistance(Mission.m_AICraftHandles[i], Mission.m_AITargetHandles[i]) < 2.0) then
                    -- Need to retarget.
                    FindGoodAITarget(i)
                end
            end
...
function UpdateAnimals()
    ...
    for i = 0, Mission.m_NumAnimals - 1 do
        if (Mission.m_AnimalHandles[i] ~= nil) then
...
function Update()
    ...
    if ((Mission.m_ScoreLimit > 0) and (not Mission.m_bDidGameOverByScore)) then
        ...
        for i = 1, MAX_TEAMS - 1 do
            ...
            if (playerH ~= nil) then
                if (Teamplay) then
                    ...
                    if ((WhichTeamGroup >= 0) and (whichteamgroupami < MAX_MULTIPLAYER_TEAMS)) then
        ...
        if (Teamplay) then
            for i = 1, MAX_MULTIPLAYER_TEAMS - 1 do
...
function RespawnPilot(DeadObjectHandle, Team)
    ...
    local RespawnInVehicle = (GetRespawnInVehicle()) or
        (Mission.m_RabbitMode and (DeadObjectHandle == Mission.m_RabbitTargetHandle))
    ...
    if (WasDeadAI) then
        ...
        for i = 0, Mission.m_NumAIUnits - 1 do
            if (DeadObjectHandle == Mission.m_AICraftHandles[i]) then
                ...
                if (Wasm_RabbitShooterHandle) then
